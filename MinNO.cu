...
int main(int argc, char** argv) {
	...
	::grpc::Status return_status = dev_stub_->get_flow_cell_info(&context1, request, &response);
	
	if(response.channel_count() == 0){
	...
	std::cerr << "Max number of channels: " << response.channel_count() << std::endl;
	...
	::minknow_api::acquisition::StopRequest stop_request1;
	...
	stop_request1.set_data_action_on_stop(action);
	
	::minknow_api::acquisition::StopResponse stop_response1;
	::grpc::Status status1 = acq_stub_->stop(&context2, stop_request1, &stop_response1);
